...
/*	Layout *l;
	...
	Layout *l;
    		for(l = (Layout *)layouts; l != layouts + LENGTH(layouts) - 1; l++)
        		if(selmon->lt[selmon->sellt] == l)
            	break;
    		if(arg->i > 0) {
        		if(l != &layouts[LENGTH(layouts) - 1] && (l + 1)->symbol)
            			setlayout(&((Arg) { .v = (l + 1) }));
        		else
            			setlayout(&((Arg) { .v = layouts }));  // wrap на начало
    			} else {
        			if(l != layouts)
            			setlayout(&((Arg) { .v = (l - 1) }));
       			 else
            			setlayout(&((Arg) { .v = &layouts[LENGTH(layouts) - 1] }));  // wrap на конец
    }
...
slidein(Client *c)
{
    ...
    for (;;) {
        ...
        float t    = (float)elapsed / duration_ms;   /* 0 → 1 */
        float ease = (1.0f - cosf(t * M_PI)) / 2.0f; /* ease-in-out */
        int cur_y  = starty + (int)((targety - starty) * ease);

        XMoveWindow(dpy, c->win, c->x, cur_y);
        XFlush(dpy);   /* << только flush, без ожидания ответа */

        sleep_ts.tv_sec  = 0;
        sleep_ts.tv_nsec = frame_ms * 1000000L;
...
slideout(Client *c)
{
    ...
    for (;;) {
        ...
        float ease = sinf(t * M_PI / 2.0f);          /* ease-out */
        int cur_y  = starty + (int)((targety - starty) * ease);
